...
app.post('/login', async function (req, res) {
  let { username, password, captcha, captchaId } = req.body
  let serverCaptcha
  for (let i = 0; i < captchaIdHistory.length; i++) {
    if (captchaId === captchaIdHistory[i].captchaId) {
      serverCaptcha = captchaIdHistory[i].text
    }
  }
  if (captcha && captchaId && captcha === serverCaptcha) {
  } else {
    res.send({
      code: 400,
      message: '验证码错误'
